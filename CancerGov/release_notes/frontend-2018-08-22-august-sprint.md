...
On some spanish pages in IE only, a white line is appearing at the very bottom of the page (below the footer).  It seems to be an issue with the <sup>®</sup> and it having a CSS reset style of vertical-align: baseline. Added vertical-align: text-top the superscript element to overwrite this and remove white line at the bottom of the page. NO CONTENT CHANGES.
...
Begin tracking package-lock.json for better out of the box dependency management.
...
Requires update to s_code in static.cancer.gov (assigned to Dion).
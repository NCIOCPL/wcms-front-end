 ...
 $.fn.topToc = function(options) {
  ...
  var defaults = {
    ...
    start: 1,                // which hN will be the first (and after it we
                             // go just deeper)
    kp: 0,
    topToc: 0,               // need to do a couple TOCTOP specials
    toptocTitle: "Contents", // what to display before our box
    viewallTitle: "View All Sections",
    ...
    viewallTitleEn: "View All Sections",
    viewallTitleEs: "Ver todas las secciones",
  ...
  var options = $.extend(defaults, options);
    ...
    if ($('meta[name="content-language"]').attr('content') == 'es' ||
        $(this).closest('[lang]').attr('lang') == 'es')
      options.toptocTitle = '<h3>' + defaults.tocTitleEs + '</h3>';
    else
      options.toptocTitle = '<h3>' + defaults.tocTitleEn + '</h3>';

    // Select the language tag to pick the proper text for
    // "View All Sections" link
    if ($('meta[name="content-language"]').attr('content') == 'es' ||
        $(this).closest('[lang]').attr('lang') == 'es')
      options.viewallTitle = defaults.viewallTitleEs;
    else
      options.viewallTitle = defaults.viewallTitleEn;
    ...
    return this.each(function() {
       ...
       while ( depth > 1) {
           start++;     //we will just get our start level and numbers
                        //higher than it
           srcTags = srcTags + ", h" + start;
           depth--;     //since went one level up, our depth will go
                        //one level down
       }
      ...
      i++;
        ...
        appHTML = appHTML + "<li class='viewall'><span>" + options.viewallTitle + "</span></li>";
    ...
    appHTML = options.toptocTitle + "<"+ options.listType + ">" + appHTML + "</" + options.listType + ">";

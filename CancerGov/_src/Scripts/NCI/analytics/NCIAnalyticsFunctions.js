...
var NCIAnalytics = {
  ...
  siteSection: (function (pagePathOverride) {
    ...
    if (oga_pattern.test(path)) {
      return ('oga');
    }
    if (cct_pattern.test(path)) {
      return ('cct');
    }
    if (pdq_pattern.test(path)) {
      return ('pdq');
    }
  ...
  GeneticServicesDirectorySearch: function (sender) {
    ...
    searchCriteria = [typeOfCancer, familyCancerSyndrome, city, state, country, lastName]
  ...
  GlobalLinkTrack: function (payload) {
    ...
    if (!clickParams.Props[48]) {
      clickParams.Props[66] = (((section) ? section + '_' : '') + label.toLowerCase());
    }
...
function changeMonitor(payload) {
  ...
  if (window[variableName] != variableValue) {
    ...
    if (variableName === 'hash' && variableValue.indexOf('#link/') > -1) {
      fireCallback = false;
    }
